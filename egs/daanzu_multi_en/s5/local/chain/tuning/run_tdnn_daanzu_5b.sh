...
if [ $stage -le 14 ]; then
  ...
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 data/lang_${multi}_${gmm}_daanzu_tg $dir $dir/graph_daanzu_tg
fi

decode_suff=daanzu_tg
graph_dir=$dir/graph_daanzu_tg

if [ $stage -le 15 ]; then
  if [ ! -z $decode_iter ]; then
    iter_opts=" --iter $decode_iter "
  fi
  if $rescore && [ ! -f data/lang_${multi}_${gmm}_daanzu_fg/G.carpa ]; then
    LM_fg=data/local/lm/4gram-mincount/lm_unpruned.gz
    utils/build_const_arpa_lm.sh $LM_fg data/lang_${multi}_${gmm}_daanzu_tg data/lang_${multi}_${gmm}_daanzu_fg
  fi
...
lang=data/lang_${multi}_${gmm}_daanzu_tg
