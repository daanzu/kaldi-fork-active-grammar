...
namespace dragonfly
{
	...
	StdFst* AgfGmmOnlineModelWrapper::ReadFstFile(std::string filename)
	{
	...
	void AgfGmmOnlineModelWrapper::StartDecoding(std::vector<bool> grammars_activity)
	{
		FreeDecoder();
	...
	void AgfGmmOnlineModelWrapper::FreeDecoder(void)
	{
		if (decoder) {
			delete decoder;
			decoder = NULL;
