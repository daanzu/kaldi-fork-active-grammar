...
namespace dragonfly
{
	...
	StdFst* AufGmmOnlineModelWrapper::ReadFstFile(std::string filename)
	{
	...
	void AufGmmOnlineModelWrapper::StartDecoding(std::vector<bool> grammars_activity)
	{
		FreeDecoder();
	...
	void AufGmmOnlineModelWrapper::FreeDecoder(void)
	{
		if (decoder) {
			delete decoder;
			decoder = NULL;
