...
ActiveGrammarFst::ActiveGrammarFst(
...
void ActiveGrammarFst::DecodeSymbol(Label label,
...
void ActiveGrammarFst::InitNonterminalMap() {
  nonterminal_map_.clear();
...
void ActiveGrammarFst::InitEntryOrReentryArcs(
...
ActiveGrammarFst::ExpandedState *ActiveGrammarFst::ExpandState(
  ...
  KALDI_ASSERT(!aiter.Done() && aiter.Value().ilabel > big_number &&
               "Something is not right; did you call PrepareForActiveGrammarFst()?");
...
// static inline
void ActiveGrammarFst::CombineArcs(const StdArc &leaving_arc,
                             const StdArc &arriving_arc,
                             float cost_correction,
                             StdArc *arc) {
  // The following assertion shouldn't fail; we ensured this in
  // PrepareForActiveGrammarFst(), search for 'olabel_problem'.
...
ActiveGrammarFst::ExpandedState *ActiveGrammarFst::ExpandStateEnd(
    int32 instance_id, BaseStateId state_id) {
  if (instance_id == 0)
  ...
  for (; !aiter.Done(); aiter.Next()) {
    ...
    if (leaving_arc.olabel != 0) {
      ...
      // PrepareForActiveGrammarFst(), or there was an error in that, because
...
int32 ActiveGrammarFst::GetChildInstanceId(int32 instance_id, int32 nonterminal,
                                     int32 state) {
...
ActiveGrammarFst::ExpandedState *ActiveGrammarFst::ExpandStateUserDefined(
    int32 instance_id, BaseStateId state_id) {
  const ConstFst<StdArc> &fst = *(instances_[instance_id].fst);
  ArcIterator<ConstFst<StdArc> > aiter(fst, state_id);
  ...
  for (; !aiter.Done(); aiter.Next()) {
    ...
    if (dest_fst_instance < 0) {
      ...
      KALDI_ERR << "Same state leaves to different FST instances "
          "(Did you use PrepareForActiveGrammarFst()?)";
    }
...
void ActiveGrammarFst::Write(std::ostream &os, bool binary) const {
  using namespace kaldi;
  if (!binary)
    KALDI_ERR << "ActiveGrammarFst::Write only supports binary mode.";
  int32 format = 1,
      num_ifsts = ifsts_.size();
  WriteToken(os, binary, "<ActiveGrammarFst>");
...
void ActiveGrammarFst::Read(std::istream &is, bool binary) {
  using namespace kaldi;
  if (!binary)
    KALDI_ERR << "ActiveGrammarFst::Read only supports binary mode.";
  if (top_fst_ != NULL)
    Destroy();
  int32 format = 1, num_ifsts;
  ExpectToken(is, binary, "<ActiveGrammarFst>");
  ReadBasicType(is, binary, &format);
  if (format != 1)
    KALDI_ERR << "This version of the code cannot read this ActiveGrammarFst, "
        "update your code.";
...
// PrepareForActiveGrammarFst(), which is declared in grammar-fst.h.
class ActiveGrammarFstPreparer {
 public:
  ...
  ActiveGrammarFstPreparer(int32 nonterm_phones_offset,
                     VectorFst<StdArc> *fst):
      nonterm_phones_offset_(nonterm_phones_offset),
 ...
 private:
  ...
  //
  // The purpose of this is to keep the ActiveGrammarFst code simple.
  //
  ...
  // of the CombineArcs() function of ActiveGrammarFst, because it needs to combine
...
bool ActiveGrammarFstPreparer::IsSpecialState(StateId s) const {
  if (fst_->Final(s).Value() == KALDI_GRAMMAR_FST_SPECIAL_WEIGHT) {
    ...
    // special cost.
    KALDI_WARN << "It looks like you are calling PrepareForActiveGrammarFst twice.";
  }
...
bool ActiveGrammarFstPreparer::IsEntryState(StateId s) const {
  int32 big_number = kNontermBigNumber,
...
bool ActiveGrammarFstPreparer::NeedEpsilons(StateId s) const {
  ...
  // in the CombineArcs() function of ActiveGrammarFst).  That function does not allow
...
void ActiveGrammarFstPreparer::FixArcsToFinalStates(StateId s) {
  int32 encoding_multiple = GetEncodingMultiple(nonterm_phones_offset_),
...
void ActiveGrammarFstPreparer::MaybeAddFinalProbToState(StateId s) {
  if (fst_->Final(s) != Weight::Zero()) {
...
void ActiveGrammarFstPreparer::GetCategoryOfArc(
...
void ActiveGrammarFstPreparer::InsertEpsilonsForState(StateId s) {
...
void PrepareForActiveGrammarFst(int32 nonterm_phones_offset,
                          VectorFst<StdArc> *fst) {
  ActiveGrammarFstPreparer p(nonterm_phones_offset, fst);
  p.Prepare();
}

void CopyToVectorFst(ActiveGrammarFst *grammar_fst,
                     VectorFst<StdArc> *vector_fst) {
  typedef ActiveGrammarFstArc::StateId GrammarStateId;  // int64
  ...
  while (!queue.empty()) {
    ...
    ArcIterator<ActiveGrammarFst> aiter(*grammar_fst, grammar_state);
    for (; !aiter.Done(); aiter.Next()) {
      const ActiveGrammarFstArc &grammar_arc = aiter.Value();
      StdArc std_arc;
